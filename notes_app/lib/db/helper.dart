...
class Helper {
  ...
  Future<Database> createOrOpen() async {
    ...
    return openDatabase(
      path,
      version: DB_VERSION,
      onCreate: (db, version) => _onTableCreated(db),singleInstance: true
    );
  }
  ...
  void _onTableCreated(Database db) {
    try {
      String sql =
          'create table $TABLE_NAME($colId integer primary key autoincrement, $colTitle text, $colText text, $colDate text, $colColor integer, $colLastUdpate text)';
      print(sql);
      db.execute(sql);
